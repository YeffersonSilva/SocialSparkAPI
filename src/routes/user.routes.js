...
router.get('/profile/:id', check.auth, profile); // Get user profile
router.get('/list/:page?', list); // List users with pagination
router.put('/update', check.auth, update); // Update user details
router.post('/upload', [check.auth, uploads.single("file0")], upload); // Upload user avatar
router.get('/avatar/:file', check.auth, avatar); // Get user avatar
